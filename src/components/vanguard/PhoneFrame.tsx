    ...
    >
      ...
      <div className="relative aspect-[9/19] rounded-[2.6rem] border border-white/15 bg-gradient-to-b from-zinc-800/80 to-black p-[6px] shadow-[0_30px_80px_-20px_rgba(0,0,0,0.8),inset_0_1px_0_rgba(255,255,255,0.08)]">
        <div className="relative h-full w-full overflow-hidden rounded-[2.2rem] bg-white">
          ...
          <div className="absolute inset-x-0 top-0 z-20 h-9 bg-white" />
          {/* Notch */}
          <div className="absolute left-1/2 top-2 z-40 h-5 w-24 -translate-x-1/2 rounded-full bg-black ring-1 ring-white/10">
            <div className="absolute right-3 top-1/2 h-1.5 w-1.5 -translate-y-1/2 rounded-full bg-zinc-700" />
          </div>
          {/* Status bar (dark text on white safe area) */}
          <div className="absolute inset-x-0 top-0 z-30 flex items-center justify-between px-5 pt-2 text-[10px] font-semibold text-zinc-700">
            <span>9:41</span>
            <span className="flex items-center gap-1">
              <span className="h-1.5 w-1.5 rounded-full bg-[var(--safe)]" /> 5G
            </span>
          </div>
          {/* Screen content (offset by safe area) */}
